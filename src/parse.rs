    ...
    pub fn new_if(cond: Node, then: Node, elthen: Option<Node>) -> Self {
        Self {
            op: match elthen {
                ...
                None => NodeType::IfThen(Box::new(cond), Box::new(then), None),
            },
        }
    }
...
fn assignment_op(tokenset: &mut TokenSet) -> Option<TokenType> {
    if tokenset.consume_ty(TokenAddEq) {
        return Some(TokenAdd);
    } else if tokenset.consume_ty(TokenSubEq) {
        return Some(TokenSub);
    } else if tokenset.consume_ty(TokenMulEq) {
        return Some(TokenStar);
    } else if tokenset.consume_ty(TokenDivEq) {
        return Some(TokenDiv);
    } else if tokenset.consume_ty(TokenModEq) {
        return Some(TokenMod);
    } else if tokenset.consume_ty(TokenShlEq) {
        return Some(TokenShl);
    } else if tokenset.consume_ty(TokenShrEq) {
        return Some(TokenShr);
    } else if tokenset.consume_ty(TokenAndEq) {
        return Some(TokenAmpersand);
    } else if tokenset.consume_ty(TokenOrEq) {
        return Some(TokenOr);
    } else if tokenset.consume_ty(TokenXorEq) {
        return Some(TokenXor);
    } else if tokenset.consume_ty(TokenAssign) {
        return Some(TokenAssign);
    } else {
        return None;
    }
}
...
fn new_ptr_to_replace_type(ctype: &Type, true_ty: Type) -> Type {
    match ctype.ty {
        ...
        _ => {
            return Type::new(
                ctype.ty.clone(),
                Some(Box::new(new_ptr_to_replace_type(
                    ctype.ptr_to.as_ref().unwrap().as_ref(),
                    true_ty,
                ))),
                ctype.ary_to.clone(),
                ctype.size,
    ...
    *ENV.lock().unwrap() = Env::new_env(None);
    ...
    loop {
        match tokenset.consume_ty(TokenEof) {
            ...
            false => {
                program.nodes.push(toplevel(tokenset));
            }
        }
    }
